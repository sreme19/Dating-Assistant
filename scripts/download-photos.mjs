 ...
 *
 * Usage:
 *   node scripts/download-photos.mjs --unsplash [UNSPLASH_API_KEY]
 *   node scripts/download-photos.mjs --replicate [REPLICATE_API_TOKEN]
 *
...
const mode = args[0]; // --unsplash or --replicate
const apiKey = args[1];

if (!mode || !apiKey) {
  console.log(`
Usage:
  node scripts/download-photos.mjs --unsplash [API_KEY]
  node scripts/download-photos.mjs --replicate [API_TOKEN]
...
async function downloadFromUnsplash(profileName, searchTerms, photoDir) {
  const photos = [];
...
async function generateWithReplicate(profileName, searchTerms, photoDir) {
  const photos = [];
  ...
  for (let i = 0; i < Math.min(5, prompts.length); i++) {
    try {
      ...
      const response = await fetchJson('https://api.replicate.com/v1/predictions', {
        ...
        body: JSON.stringify({
          version: 'ac732df83cea7fff18b51f390b87a8f09df4fabf3b3b1ac3db8e1c7b2d94cc26', // Stable Diffusion 1.5
          input: {
            prompt,
            num_outputs: 1,
...
async function main() {
  ...
  for (const profile of profiles) {
    // Generate search terms based on archetype
    let terms = searchTerms[`${profile.name}_${profile.archetype.split(' ')[0]}`] ||
                searchTerms[`${profile.name}`] ||
                [`${profile.name}, ${profile.archetype}`];

    console.log(`📁 ${profile.name} (${profile.archetype})`);

    let count = 0;
    if (mode === '--unsplash') {
      count = await downloadFromUnsplash(profile.name, terms, profile.photoDir);
    } else if (mode === '--replicate') {
      count = await generateWithReplicate(profile.name, terms, profile.photoDir);
    }
